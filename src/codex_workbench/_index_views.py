...
def _render_current(snapshot: _IndexSnapshot) -> str:
    ...
    if remaining_actionable > 0:
        lines.extend(
            [
                "",
                f"> 还有 {remaining_actionable} 个可推进任务未展示；查 `docs/generated/index.md`。",
            ]
        )
    lines.extend(["", "## 等待反馈", ""])
    lines.extend(
    ...
    if remaining_waiting > 0:
        lines.extend(
            ["", f"> 还有 {remaining_waiting} 个等待反馈任务未展示；查 `docs/generated/index.md`。"]
        )
    lines.extend(
        [
            "",
    ...
    *,
    ...
    for task in tasks:
        ...
        if isinstance(validation, dict):
            validation_status = (
                str(validation.get("status", "not_started")).strip() or "not_started"
            )
...
def _waiting_feedback_recovery_lines(
    task: _YamlRecord, requirement_by_id: dict[str, _YamlRecord]
) -> list[str]:
    data = task.data or {}
    stage = str(data.get("stage", "unknown")).strip() or "unknown"
    requirement_id = str(data.get("requirement_id", "")).strip()
...
def _archive_lines(records: list[_YamlRecord]) -> list[str]:
    ...
    for record in sorted(
        records, key=lambda item: str(item.data.get("version", "")) if item.data else ""
    ):
        if not record.data:
            continue
        try:
...
def _task_packet_status(task: _YamlRecord) -> str:
    ...
    if (
        _has_nonempty_file(task.path.parent / "evidence.md")
        or (task.path.parent / "evidence.yaml").exists()
    ):
