...
## 1. 让 Codex 安装 Workbench

最推荐的安装方式不是你自己照着命令敲，而是把下面这段话复制给 Codex，让 Codex 帮你安装和检查。
...
先把下面提示词里的 `<你的 Workbench 目录>` 改成你自己的本地目录，再复制给 Codex：

```text
请帮我安装并检查 codex-workbench。

目标目录：<你的 Workbench 目录>
仓库地址：https://github.com/Beautiful12138/codex-workbench.git

要求：
1. 如果目标目录不存在，请 clone 仓库到该目录；如果目录已存在，只检查当前仓库状态，不要覆盖我的改动。
2. 使用当前用户的 Python 环境安装 Workbench 依赖，不创建本仓库专用环境，也不要临时绕过已安装命令。
3. 确认 codex-workbench 命令可用；如果 Python 用户 Scripts 目录不在 PATH，请帮我加入用户 PATH，并提醒我重启 Codex Desktop 或新开终端来刷新 PATH。
4. 运行 codex-workbench version。
5. 运行 codex-workbench workspace context --workspace-root <你的 Workbench 目录>。
6. 最后告诉我安装是否成功、你验证了什么、还有哪些事情需要我手动确认。
```

如果 Codex 修改了 PATH，当前已经打开的 Codex Desktop 或终端可能还看不到新路径。重启 Codex Desktop，或新开一个终端后再继续使用。
...
如果你希望 Workbench 在工作日自动整理可复用经验，需要在 Codex Desktop 的“已安排”里创建一个自动化。推荐让 Codex 帮你创建，因为它能直接把工作空间、模型、推理等级和提示词放进去。

先把下面提示词里的 `<你的 Workbench 目录>` 改成你自己的本地目录，再复制给 Codex：

```text
请帮我在 Codex Desktop 的“已安排”里创建一个 Workbench 可复用经验维护自动化。
...
创建后请读取自动化配置，确认工作空间、模型、推理强度、时间和状态都正确。
```
