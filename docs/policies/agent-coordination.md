...
本文件说明 Codex、skills、CLI、子代理和用户如何协作。目标是让 Codex 把注意力放在任务本身，同时在需要时能自然发现规则、命令和状态真源。
...
- policy：运行规则、边界和反例。
- skills：日常行动手册，告诉 Codex 怎么读、怎么判断、什么时候停、用哪个命令。
- CLI/schema：可靠写入和门禁。
...
skill 不替代包 YAML、evidence、CLI 或用户确认。它只负责把当前场景连到正确行动路径上。
...
子代理结论不是事实真源。主代理必须核对文件、命令输出和状态包；不得仅凭子代理结论写状态、推进阶段、声明完成或执行高影响操作。

高风险、critical 或有真实后果的任务，不应由同一主体自写自审通过；优先让子代理做独立复核，必要时由用户、外部人或其他独立主体确认。
