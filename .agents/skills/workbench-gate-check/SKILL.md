...
description: 阶段推进前自检/能不能 set-stage/进入 in_progress/能不能 done/task check 过不了/blocked 还是继续。Use when Codex 需要只读判断 codex-workbench 阶段门禁、推进条件或 set-stage 到 verification_pending/done/obsolete。
---
...
```powershell
codex-workbench task check <TASK-ID> --to in_progress
codex-workbench task check <TASK-ID> --to done
```

写入阶段前，再次确认用户授权和门禁结果：

```powershell
codex-workbench task set-stage <TASK-ID> --stage in_progress
codex-workbench task set-stage <TASK-ID> --stage done
```
