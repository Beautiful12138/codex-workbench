...
- 判断 task 能不能进入 `in_progress`。
- 判断 task 能不能进入 `verification_pending` 或 `done`。
- 判断 task 应该 `blocked` 还是继续推进。
- 判断误建或废弃任务能不能 `obsolete`。
- 用户问“能不能继续”“能不能完成”“下一步是不是推进阶段”。

## 核心原则

gate-check 是只读自检，不写状态。它帮助 Codex 在执行 CLI 之前确认事实、目标、风险、验证和交接是否够清楚。
...
1. 读 `AGENTS.md`，必要时运行 `workspace context` 确认工作对象。
2. 先用 `task context <任务名或ID>` 看当前缺口。
3. 需要阶段写入时，再读目标 task 的 `task.yaml`、`task.md` 和 `docs/policies/state-and-gates.md`。
4. 若涉及风险、unknown、高风险或真实后果，读 `docs/policies/risk-and-process.md`。
5. 若涉及验证或 done，读 `evidence.yaml`、validation、handoff，并按需读 `workbench-evidence`。
6. 若涉及环境、数据、权限、部署或外部系统，读 `workbench-environment` 和相关 `environments/` Markdown。
...
| 目标 | 重点检查 |
| --- | --- |
| `ready` | 是否有 readable requirement、目标和完成口径是否清楚 |
| `in_progress` | 是否 implementation-ready、working_scope、risk_triggers、验证和回滚清楚 |
| `verification_pending` | 是否已完成实现，是否准备记录真实验证 |
| `done` | 是否有 evidence、validation passed、handoff 不等待不拒绝 |
...
- high/critical 或真实后果任务有 review done、implementation ref 和风险接受。

缺任何关键项时，不要推进到 `in_progress`。先补 `task prepare`、`task impact-set`、review / implementation，或暂停确认。
...
- evidence 不能只记录 gate-check 输出；gate-check 只能解释当前为什么能或不能推进。
...
- 阻塞原因具体。
- 阻塞方明确，例如 user、external_env、missing_info、third_party。
- 恢复条件可执行。
- 恢复后阶段明确。
