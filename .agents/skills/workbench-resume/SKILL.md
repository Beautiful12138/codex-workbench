...
`AGENTS.md` 是稳定入口规则卡。日常恢复默认先用 `workspace context`，再进入 `task context`、`service context` 和任务包细节。`CURRENT.md` 是 CLI 生成的最近工作面板，不是完整手册，也不作为单任务锁。工作对象选择必须来自用户请求、显式路径、名称、ID、`CURRENT.md`、`docs/generated/recovery.md`、`docs/generated/index.md` 或包 YAML。
...
风险摘要和缺口只用于恢复注意力，不替代 task YAML。遇到 high/critical、`environment=unknown`、真实数据、生产/共享环境、权限安全、部署、不可逆、影响他人、验证或回滚不清的任务，不能只靠 `CURRENT.md` 继续推进，必须读取 task 包和 `docs/policies/risk-and-process.md`。
...
2. 运行 `workspace context --workspace-root .`，获得轻量驾驶舱。
3. 若用户给出显式路径、需求/任务名称、REQ/TASK ID、服务名，优先读取对应对象。
4. 若没有明确对象，再读 `CURRENT.md`、`docs/generated/recovery.md`；需要完整活动目录时读 `docs/generated/index.md`。
5. 若选择到 task，优先运行 `task context <任务名或ID>` 获得轻量工作面板；需要细节时再读 `docs/active/<TASK-ID>/task.yaml` 和 `task.md`。
6. 若涉及服务，优先运行 `service context <服务名>`；需要完整登记时再读 `services/registry.yaml`。
7. 若涉及环境、服务器、数据库、GitLab、网站、联调、账号密码或操作方式，查 `environments/` 中相关 Markdown。
8. 若涉及验证、done 或归档，读 evidence、handoff 和对应 policy。
9. 若 task 显示 high/critical、unknown 或真实后果风险，读 `docs/policies/risk-and-process.md` 和 task 的 review / implementation 信息。
...
回答“下一步做什么”时，如果涉及实际操作，先检查是否需要 `workbench-cli`。需要写状态、推进阶段、生成视图、检查状态或归档时，先读 `workbench-cli` 并用 `--help` 核对命令。
...
- 等待反馈任务，例如 `verification_pending` 或 `waiting_user_validation`；这不是阻塞。
- blocked 任务。
- high/critical、unknown 或真实后果风险的任务。
- generated views 中显示风险缺口的任务。
- 已登记服务和缺失服务。
- 相关环境资料是否存在。
- 当前允许动作。
- 当前禁止动作。
...
回复用户时使用需求/任务名称；ID 只在歧义、调试、复制 CLI 命令或用户明确要求时出现。
