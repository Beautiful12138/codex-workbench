...
description: 继续刚才那个/当前状态/当前到哪了/下一步/有哪些任务/上次做到哪/先看工作台全貌。Use when Codex 需要恢复 codex-workbench 现场、选择工作对象、处理聊天/只读探索/模糊指代/是否需要写状态判断。
---
...
3. 用户问已有材料、说明文档、架构/流程梳理，或要求搬迁材料时，先查 `docs/materials/` 和 `docs/briefs/`；外部/用户提供内容的搬迁副本放 `docs/materials/`，Codex 生成的梳理说明放 `docs/briefs/`，不因此写状态。
4. 用户给出显式路径、需求/任务名称、REQ/TASK ID、服务名时，优先读取对应对象。
5. 仍不清楚对象时，再读 `CURRENT.md`、`docs/generated/recovery.md`；需要完整活动目录时才读 `docs/generated/index.md`。
6. 选择到 task 后，优先运行 `task context <任务名或ID>` 看当前能做什么、缺什么；需要细节时再读 `docs/active/<TASK-ID>/task.yaml` 和 `task.md`。
7. 涉及服务时，优先运行 `service context <服务名>`；需要完整登记时再读 `services/registry.yaml`。
8. 涉及环境、服务器、数据库、GitLab、网站、联调、账号密码或操作方式时，查 `environments/` 中相关 Markdown。
9. 涉及验证、done、归档、高风险、unknown 或真实后果时，再读 evidence、handoff、policy、review / implementation 信息。
...
codex-workbench task context <TASK-NAME-OR-ID> --workspace-root .
codex-workbench service context <SERVICE-NAME> --workspace-root .
