...
`AGENTS.md` 是稳定入口规则卡。`CURRENT.md` 是 CLI 生成的最近工作面板，不是完整手册，也不作为单任务锁。工作对象选择必须来自用户请求、显式路径、ID、`CURRENT.md`、`docs/generated/recovery.md`、`docs/generated/index.md` 或包 YAML。
...
2. 读 `CURRENT.md`，确认最近更新的活动任务。
3. 若用户给出显式路径、REQ/TASK ID、服务名，优先读取对应对象。
4. 若没有明确对象，读 `docs/generated/recovery.md`；需要完整活动目录时读 `docs/generated/index.md`。
...
8. 若 task 显示 high/critical、unknown 或真实后果风险，读 `docs/policies/risk-and-process.md` 和 task 的 review / implementation 信息。
