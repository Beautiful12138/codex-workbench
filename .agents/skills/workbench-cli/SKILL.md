---
name: workbench-cli
description: 写状态命令怎么跑/命令参数/Workbench CLI --help/workspace、task、service context/doctor、index、generated views 报错。Use when Codex 只需发现或核对 codex-workbench CLI 命令边界。
---
...
从仓库根目录运行 CLI 时使用用户 Python 环境中已安装的命令：

```powershell
codex-workbench <group> <command> --help
```

不知道命令在哪个组时，用：

```powershell
codex-workbench --help
codex-workbench task --help
```

不要临时绕过已安装命令。先读本 skill，再用 `codex-workbench --help` 核对当前 CLI；如果命令不存在，先处理安装或 PATH，而不是切回源码运行。
...
| 看某个项目的服务 | `workspace context --project <项目名>` | 是否需要点名 `service context` 查看细节 |
...
- `service` / `workspace`：工作区和服务上下文，核心命令 `workspace context`、`workspace context --project/--ungrouped`、`service add/update/delete`、`service context`；服务项目分组用 `service add/update --project` 设置，用 `service update --clear-project` 清除；`service status` 主要用于调试和脚本。
...
`workspace context` 默认只读 registry，按项目分组全量列出服务名称，不显示路径、用途、备注，也不深扫 Git 或入口文件。可用 `--project <项目名>` 聚焦项目，用 `--ungrouped` 聚焦未分组服务。需要真实服务现场时，优先点名 `service context <服务名>`；确实需要批量探测时才用 `workspace context --check-services`，且全局或指定项目都最多深入检查 5 个服务。
